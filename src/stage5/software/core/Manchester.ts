...
import { LastMessageHandlerPlugin, ListenMessageCallback } from './MessageHandler';

const preData: NetData[] = getNetData('0000001100110011');
const endData: NetData[] = getNetData('00000000');

export class Manchester implements LastMessageHandlerPlugin {
      ...
      if (flag) {
        ...
        if (state === last[now]) {
          ...
          if (now === last.length) {
            flag = false;
            now = 0;
            const translateRes = this.translate(res);
            cb(translateRes);
            res = [];
          }
        } else {
